...
| 30 | ✅ **Conversation word count in chat header** | Done: `conversationStats` useMemo computes word count and exchange count across non-compact messages; shown in the chat tab subtitle (replaces the static description text when a conversation is active). |
...
| 32 | ✅ **Draft prompt auto-save on refresh** | Already implemented: `useState` initializes from `localStorage.getItem('botty.draftPrompt')` and a `useEffect` saves on every change. Draft persists across page refreshes when no conversation is active. |
| 33 | ✅ **Fact import from text file** | Done: “Import” button next to Add in the Memory facts form; hidden file input accepts `.txt`/`.md`; splits by newline, POSTs each non-empty line as a fact, shows `Imported N facts from filename` notice. |
